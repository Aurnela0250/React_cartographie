...
import { RegionFilter } from "@/core/filters/region.filter";
...
export class RegionApiRepository implements IRegionRepository {
    async filter(
        ...
        const url = `${env.API_PREFIX_URL}/${env.API_VERSION}/regions/filter/?${new URLSearchParams(toSnakeCaseRecursive(cleanedFilters as Record<string, string>)).toString()}`;
    ...
    async create(token: string, data: { name: string }): Promise<Region> {
